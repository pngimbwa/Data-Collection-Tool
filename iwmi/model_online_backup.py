...
class RelationManager(models.Model):
    ...
    personA = models.CharField('Person A',max_length=25)
    personB = models.CharField('Person B',max_length=25)
    relation = models.CharField('Relation',max_length=20)


class Farm(models.Model):
    fieldID = models.CharField('FieldID',primary_key=True, max_length=40)
    latitude = models.FloatField()
    longitude = models.FloatField()
    #crop = models.ForeignKey(Crop, verbose_name="Crop(s)")
    farmer = models.ForeignKey(People, verbose_name="Farmer")
...
class Plot(models.Model):
    plotID = models.CharField('Plot ID',max_length=50)
    farm = models.ForeignKey(Farm,verbose_name='FieldID')
    village = models.ForeignKey(Village, verbose_name="Village")
    
    class Meta:
        ordering = ['plotID']
        unique_together = ('village','plotID')
...
class PlotManagement(models.Model):
    ...
    #plot_number = models.IntegerField('Plot Number')
    ...
    #pumping_source = models.CharField('Pumping source',max_length = 80)#,choices=PUMPING_SOURCE_CHOICES)
...
class CropPropertyFarm(models.Model):
    farm = models.ForeignKey(Farm,verbose_name='Farm')
...
class TechnologyManagement(models.Model):
    ...
    farmer = models.ForeignKey(People,verbose_name='Farmer',null=True,blank=True)
...
class TechnologyCalibration(models.Model):
    ...
    bucketvolume = models.FloatField('Bucket volume (litre)')
...
class FertilizerManagement(models.Model):
    ...
    #personels = models.ManyToManyField(People,verbose_name="Applied Individual(s)")
    ...
    fertilizer_management = models.CharField('Fertilizer Management',max_length = 75)#,choices=FERTILIZER_MANAGEMENT_CHOICES)
    #fertilizer_types = models.CharField('Fertilizer types',max_length = 45)#,choices=FERTILIZER_TYPES_CHOICES)
...
class FarmIrrigationEvent(models.Model):
    CLIMATE_CHOICES=(
        ('Rainy','Rainy'),
        ('Sunny','Sunny'),
        ('Cloudy','Cloudy'),
...
class Harvest(models.Model): 
    ...
    farm = models.ForeignKey(Farm,verbose_name='Harvested farm')
    time = models.FloatField('Time taken')
    ...
    def __str__(self):
        return '{},{}'.format(self.date,self.farm)
'''
